...
def _try_fill_strip(y_min, y_max, spot_w, min_x, max_x, rotated_usable, road_network, road_network_tmp):
    placed_spots = []
    placed_spots_union = shapely.Polygon()
    x = min_x
    while x < max_x:
        spot_candidate = shapely.box(x, y_min, x + spot_w, y_max)
        if rotated_usable.contains(spot_candidate):
            if spot_candidate.intersection(road_network_tmp).area < 0.01:
                shared_edge_len = spot_candidate.intersection(road_network).length
                if shared_edge_len >= 2.0:
...
def optimize_layout_v2(outline, blocked_zones, entrance):
    """
    Layout generation with mixed spot orientations (perpendicular/parallel) per strip.
    """
    outline_m = [(p[0] / 10.0, p[1] / 10.0) for p in outline]
    blocked_m = [[(p[0] / 10.0, p[1] / 10.0) for p in zone] for zone in blocked_zones]
    entrance_m = (entrance[0] / 10.0, entrance[1] / 10.0)
    ...
    for angle in angles:
        ...
        for offset in offsets:
            ...
            for road in horiz_roads:
                ...
                # Strip above
                spots_above_perp = _try_fill_strip(ry + 2.5, ry + 7.5, 2.5, min_x, max_x, rotated_usable, road_network, road_network_tmp)
                spots_above_para = _try_fill_strip(ry + 2.5, ry + 5.0, 5.0, min_x, max_x, rotated_usable, road_network, road_network_tmp)
                if len(spots_above_perp) >= len(spots_above_para):
                    placed_spots.extend(spots_above_perp)
                else:
                    placed_spots.extend(spots_above_para)

                # Strip below
                spots_below_perp = _try_fill_strip(ry - 7.5, ry - 2.5, 2.5, min_x, max_x, rotated_usable, road_network, road_network_tmp)
                spots_below_para = _try_fill_strip(ry - 5.0, ry - 2.5, 5.0, min_x, max_x, rotated_usable, road_network, road_network_tmp)
                if len(spots_below_perp) >= len(spots_below_para):
                    placed_spots.extend(spots_below_perp)
                else:
                    placed_spots.extend(spots_below_para)
    ...
    basement_area = outline_poly.area
    road_area = final_roads_geom.area if not final_roads_geom.is_empty else 0.0
    spot_area = len(best_spots) * 12.5  # each spot = 12.5 m^2 (2.5x5.0 or 5.0x2.5)
    efficiency = spot_area / basement_area if basement_area > 0 else 0.0
